...
plt.ylabel('')
plt.title('Unsicherheit (Elemente aus Matrix $P$)')
...
fig = plt.figure(figsize=(16,9))
plt.step(range(len(measurements[0])),Zx, label='$\dot x$')
plt.step(range(len(measurements[0])),Zy, label='$\dot y$')

plt.xlabel('Filter Step')
plt.ylabel('')
plt.title('Messungen (Elemente aus Messvektor $Z$)')
plt.legend(loc='best',prop={'size':22})
